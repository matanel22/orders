...
const DeteilsCard = ({
  ...
  return (
    <CardContainer key={id}>
        ...
        <div><Span>סוג אירוע:</Span>{`${eventType}`}</div>
        <div ><Span>מקום אירוע:</Span>{locationType}</div>
        <div><Span>סטטוס:</Span>{statusId.name}</div>
        <div><Span> תאריך הספקה:</Span>{orderDate?.toLocaleDateString()}</div>
        <div><Span> זמן הספקה:</Span>{orderTime}</div>
        {/* <div><span>סוג אירוע</span>{statusId.name}</div> */}
        <div>{items.map((item)=>{
          return <div key={item.id}><ItemsDetails id={item.id} name={item.name} amount={item.amount} /></div>

        })}</div>
        </div>
        <div style={{display:"flex",justifyContent:"end",paddingLeft:"1rem"}}>
